...
export default function BuatKelas() {
  ...
  useEffect(() => {
    ...
    async function fetchObservers() {
      const { data, error } = await supabase
        .from('users')
        .select('*')
        .eq('role', 'observer'); // Hanya ambil yang rolenya observer
      
      if (!error && data) {
        setObserversData(data);
      }
  ...
  const filtered = observersData.filter(o =>
          ...
          <div className="mb-6">
            ...
            <div className="border-2 border-slate-200 rounded-xl p-3">
              ...
              <div className="flex flex-col gap-1 max-h-48 overflow-y-auto">
                {loadingObservers ? (
                  <div className="text-center text-sm text-slate-400 py-4">Memuat data observer dari database...</div>
                ) : filtered.length === 0 ? (
                  <div className="text-center text-sm text-slate-400 py-4">Observer tidak ditemukan.</div>
                ) : (
                  filtered.map(obs => (
                    <label key={obs.id}
